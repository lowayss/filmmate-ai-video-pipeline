...
(function(global){
  ...
  function fallbackCue(shot){
    ...
    return {
      goal:'정본에 지정된 중심 행동을 수행한다',subtext:'콘티에 없는 내면 동기나 감정을 추가하지 않는다',
      trigger:shot?.new_information||'정본에 지정된 사건',target:shot?.continuity?.gaze_target||'정본에 지정된 시선 대상',
      tactic:shot?.screen?.subject_action||shot?.shot_intent||'지정된 중심 행동을 수행한다',
      fac:faceCritical?'콘티에 명시된 관찰 가능한 얼굴 변화만 사용한다. AU를 추가로 발명하지 않는다.':'얼굴이 핵심 프레임에 없으므로 FACS를 적용하지 않는다.',
      visible:shot?.screen?.subject_action||shot?.shot_intent||'지정된 중심 행동만 화면에 보인다.',
      body:'호흡·손·고개 중 정본에 지정된 신호만 먼저 반응한다.',recovery:shot?.continuity?.state_out||'다음 엔드스테이트로 회복한다',faceCritical,expressionEvent:false
    };
  }

  function isModori(context){
    return String(context?.projectTitle||'')==='모서리' || String(context?.projectTitle||'').includes('모서리');
  }

  function cueFor(shot,context){
    const id=String(shot?.id||'');
    return isModori(context) ? (modoriByShot[id]||fallbackCue(shot)) : fallbackCue(shot);
  }

  function render(shot,context={}){
    const cue=cueFor(shot,context);
    if(cue.faceCritical===false){
      return `표정·연기(관찰 가능한 지시): ${cue.visible}\n몸·호흡: ${cue.body}\n회복 상태: ${cue.recovery}`;
    }
    const exact=Boolean(context.hasExactTiming);
    const hasExpressionEvent=cue.expressionEvent!==false&&!String(cue.fac||'').includes('AU 없음');
    const timing=!hasExpressionEvent
      ? '정본에 표정 변화 사건이 없으므로 기준 상태를 유지한다. 임의의 onset·apex·offset을 만들지 않는다.'
      : exact
        ? `기준 상태에서 ${context.start}에 작은 onset → ${context.apex}에 짧은 apex/유지 → ${context.offset}부터 offset → ${context.end}까지 회복한다.`
        : '기준 상태에서 시작한다. 컷별 정확한 초 단위 시각은 정본에 없으므로 사건 직후 낮은 강도로 onset하고, 짧게 유지한 뒤 offset한다. 정점 뒤 작은 잔여 반응을 남긴다.';
